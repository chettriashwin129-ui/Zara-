...
code = code.replace(/\}\)\.catch\(err => console\.error\(err\)\);\.catch\(err => console\.error\("DB Save Error:", err\)\);\.catch\(err => console\.error\("DB Save Error:", err\)\);/g, "}).catch(err => console.error(err));");
