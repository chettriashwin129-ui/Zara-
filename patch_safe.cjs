...
const idx1 = code.indexOf('          systemInstruction: `You are Zara AI, designed and developed by Ashwin Chhetri as his personal AI companion');
if (idx1 !== -1) {
  const endIdx1 = code.indexOf('- Build long-term context so every conversation feels like continuing an existing relationship rather than starting over.', idx1) + '- Build long-term context so every conversation feels like continuing an existing relationship rather than starting over.'.length;
  code = code.substring(0, idx1) + '          systemInstruction: `' + personaBase + code.substring(endIdx1);
}
